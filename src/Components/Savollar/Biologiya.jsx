...
const Biologiya = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get('https://testbotbackend-7-ftxu.onrender.com/test/answer/get');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setCurrentQuestionIndex(currentQuestionIndex + 1); // Mos ravishda indeksni o'zgartiring
      setNatija(response.data);
      if (currentQuestionIndex === questions.length - 1) { // Misol: Son keyingi savolga o'tishdan oldin
        navigate('/javob');
      }
    } catch (error) {
