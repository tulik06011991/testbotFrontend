...
const adminSavol = () => {
  ...
  useEffect(() => {
    ...
    const admin = async () =>{
      try {
        ...
        const response = await axios.get(`https://testbotbackend-7.onrender.com/foydalanuvchi/user/${id}`,
        {
          headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await axios.post('https://testbotbackend-7.onrender.com/questions/adminPost', {
        title,
        options,
        correctAnswer
      },  
